$(function(){
    $("#show_menu").attr('src','html/user.html');
})
function openli(data){
    $("#context").remove();
    let iframe=$("#show_menu");
    iframe.attr('src','html/'+data+'.html');
}
!function (e, t, a) {
    function r() {
        for (var e = 0; e < s.length; e++) s[e].alpha <= 0 ?
            ...
            (s[e].y--, s[e].scale += .004,
                s[e].alpha -= .013,
                s[e].el.style.cssText = "left:" +
                    s[e].x + "px;top:" +
                    s[e].y + "px;opacity:" +
                    s[e].alpha + ";transform:scale(" +
                    s[e].scale + "," +
                    s[e].scale + ") rotate(45deg);background:" +
    ...
    e.requestAnimationFrame = e.requestAnimationFrame ||
        ...
        i(".heart{" +
            ...
            "}." +
            "heart:after,.heart:before{content: '';" +
            "width: inherit;" +
            "height: inherit;background: inherit;" +
            "border-radius: 50%;" +
